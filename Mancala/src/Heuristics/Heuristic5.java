...
public class Heuristic5 implements Heuristic{
    ...
    public int evaluateState(State state, int playerNumber) {
        return w1*state.getStorageDiff(playerNumber)+w2*state.getCloseToStorage(playerNumber)+w3*state.extraTurn(playerNumber)+w4*state.numStoneCaptured(playerNumber);
    }
}
